<template>
  <section
    ...
    <div class="flex-1 mt-30 w-full relative z-10">
      ...
      <!-- Action Buttons -->
      <div ref="buttonGroupRef" class="flex flex-col sm:flex-row items-center  sm:items-center gap-6 mb-8 opacity-0">
        <!-- Primary CTA -->
        <div class="relative group">
          ...
          <a href="#contact" class="relative cursor-pointer flex items-center gap-3 px-3 py-3 md:px-6 md:py-3 lg:px-8 lg:py-4 bg-slate-900 border border-cyan-500/50 rounded-xl text-white font-semibold hover:bg-slate-800 transition-all duration-300 group-hover:scale-105">
            <Zap class="w-4 h-4 md:w-5 md:h-5 text-cyan-400" />
            <span class="text-[0.7rem] lg:text-[1rem]">INITIATE CONTACT</span>
        ...
        <div class="relative group">
          ...
          class="flex items-center cursor-pointer gap-3 px-3 py-3 md:px-6 md:py-3 lg:px-8 lg:py-4 bg-transparent border-2 border-slate-600 rounded-xl text-slate-300 font-semibold hover:border-emerald-500 hover:text-emerald-400 transition-all duration-300 group-hover:scale-105">
            <FileText class="w-4 h-4 md:w-5 md:h-5" />
            <span class="text-[0.7rem] lg:text-[1rem]">ACCESS RESUME</span>
      ...
      <div ref="socialRef" class="flex items-center gap-6 opacity-0">
        <span class="text-slate-500 text-xs md:text-sm font-mono">CONNECT_TO:</span>
        <div class="flex gap-4">
          <button
            v-for="social in socialLinks"
            :key="social.name"
            @click="$emit('open-social', social.name)"
            class="group relative p-2 md:p-3 bg-slate-900/50 backdrop-blur-sm border border-slate-700 rounded-xl hover:border-cyan-500/50 transition-all duration-300 hover:scale-110">
            <component :is="social.icon" class="w-4 h-4 md:w-5 md:h-5 text-slate-400 group-hover:text-cyan-400 transition-colors duration-300" />
    ...
    <!-- Avatar Section -->
    <div class="flex-shrink-0 mt-22 lg:mt-0 lg:mr-12">
      <div ref="imageGroupRef" class="relative opacity-0">
        ...
        <div class="relative hidden lg:flex w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96 rounded-full overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-4 border-transparent bg-clip-padding">
          <!-- Gradient Border -->
          <div class="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 p-1">
            <div class="w-full h-full rounded-full bg-slate-900 overflow-hidden">
              <img
                ...
                class="sm:w-75 sm:h-100 md:w-75 md:h-100 lg:w-full object-cover filter contrast-110 brightness-90 hover:brightness-100 transition-all duration-500 hover:scale-105" />
            </div>
          </div>
